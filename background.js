...
chrome.contextMenus.onClicked.addListener(async function (info, tab) {
  if (info.menuItemId === 'summarize' || info.menuItemId === 'comment') {
    ...
    chrome.tabs.executeScript(tab.id, { file: 'content.js' }, async function () {
      ...
      if (!response.ok) {
  const responseBody = await response.json();  // Extract the response body
  console.error('API Response Error:', response.statusText);
  console.log('Full API Response:', responseBody);  // Log the extracted response body
  return;
}
        ...
        if (resultData.errors) {
          console.error('API Error:', resultData.errors);
          return;
        }
